# 🚀 Actividades Prácticas

Esta carpeta contiene todas las actividades prácticas, talleres y desarrollos realizados durante el curso de Inteligencia Artificial.

## 📚 Registro de Actividades

| Actividad | Carpeta / Enlace | Descripción |
| :-------: | :--------------- | :---------- |
| **Actividad 1** | [`/01_Prueba_inicial_streamlit`](./01_Prueba_inicial_streamlit/) | Desarrollo y creación de nuestra primera aplicación utilizando Streamlit. |
| **Actividad 2** | [`/02_Lecture02_algoritmos_busqueda_grafo`](./02_Lecture02_algoritmos_busqueda_grafo/) | Implementación y análisis de distintos algoritmos de búsqueda sobre grafos, estudiando su funcionamiento y comportamiento. |
| **Actividad 3** | [`/03_Lecture02_busqueda_en_laberintos`](./03_Lecture02_busqueda_en_laberintos/) | Aplicación de algoritmos de búsqueda para resolver problemas de laberintos y encontrar caminos desde un estado inicial hasta un estado objetivo. |
| **Actividad 4** | [`/04_Lecture03_hill_climbing`](./04_Lecture03_hill_climbing/) | Implementación del algoritmo Hill Climbing como técnica de búsqueda local y optimización. |
| **Actividad 5** | [`/05_Lecture03_simulated_annealing_hospitales`](./05_Lecture03_simulated_annealing_hospitales/) | Aplicación del algoritmo Simulated Annealing a un problema de optimización relacionado con la ubicación de hospitales. |
| **Actividad 6** | [`/06_Lecture03_algoritmos_geneticos`](./06_Lecture03_algoritmos_geneticos/) | Implementación y estudio de algoritmos genéticos como técnica de búsqueda y optimización inspirada en la evolución natural. |
| **Actividad 7** | [`/07_Lecture03_algoritmos_geneticos_hospitales`](./07_Lecture03_algoritmos_geneticos_hospitales/) | Aplicación de algoritmos genéticos al problema de optimización de ubicación de hospitales. |
| **Actividad 8** | [`/08_Lecture03_programacion_lineal`](./08_Lecture03_programacion_lineal/) | Formulación y solución de problemas de optimización mediante programación lineal. |
| **Actividad 9** | [`/09_Lecture03_csp`](./09_Lecture03_csp/) | Modelamiento y resolución de Problemas de Satisfacción de Restricciones (CSP) mediante Backtracking, MRV, Forward Checking y LCV. |
| **Actividad 10** | [`/10_Lecture03_workshop_reinas_optimizacions`](./10_Lecture03_workshop_reinas_optimizacion/) | Modelamiento y resolución del problema de las 4 reinas mediante técnicas de optimización local: Hill Climbing, Random Restart y Simulated Annealing. |


---

## 📝 Nota

Este registro se actualizará a medida que se desarrollen nuevas actividades, talleres y ejercicios durante el curso.
